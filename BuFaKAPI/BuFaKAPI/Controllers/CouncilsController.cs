namespace BuFaKAPI.Controllers
{
    ...
    public class CouncilsController : ControllerBase
    {
        ...
        public CouncilsController(MyContext context)
        {
        ...
        [HttpGet]
        public IActionResult GetCouncil([FromQuery] string apikey)
        {
            // TODO Permission Level User
            var auth = new AuthService(this.context);
            if (auth.KeyIsValid(apikey))
            {
                return this.Ok(this.context.Council);
            }
        ...
        [HttpGet("{id}")]
        public async Task<IActionResult> GetCouncil([FromRoute] int id, [FromQuery] string apikey)
        {
            // TODO Permission Level User
            var auth = new AuthService(this.context);
            if (auth.KeyIsValid(apikey))
            {
                ...
                var council = await this.context.Council.FindAsync(id);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutCouncil([FromRoute] int id, [FromBody] Council council, [FromQuery] string apikey)
        {
            ...
            if (auth.KeyIsValid(apikey))
            {
                ...
                this.context.Entry(council).State = EntityState.Modified;
        ...
        /// <response code="401">If API Key is not valid</response>
        [HttpPost]
        public async Task<IActionResult> PostCouncil([FromBody] Council council, [FromQuery] string apikey)
        {
            // TODO Permission Level SuperAdmin
            var auth = new AuthService(this.context);
            if (auth.KeyIsValid(apikey))
            {
                ...
                this.context.Council.Add(council);
                await this.context.SaveChangesAsync();
        ...
        {
            return this.context.Council.Any(e => e.CouncilID == id);
        }
    }
}